...
func main() {
	...
	funcs, err := analyzeCode(scope, docs)
	if err != nil {
		log.Fatal(err)
	}

	if err := generateCode(funcs); err != nil {
		log.Fatal(err)
	}
}

func generateCode(funcs []testFunc) error {
...
// format returns a formatted version of src with imports corrected.
func format(src []byte) ([]byte, error) {
	// Best guess at filename, fallback to output.go.
	filename := *out
	...
	buf, err := imports.Process(filename, src, nil)
	if err != nil {
		return nil, fmt.Errorf("import processing %q failed: %w\n%s", filename, err, string(src))
	}
...
func analyzeCode(scope *types.Scope, docs *doc.Package) ([]testFunc, error) {
...
var headerTemplate = `// Code generated by github.com/stretchr/testify/_codegen DO NOT EDIT.
