...
func IsOrdered(t TestingT, object interface{}, msgAndArgs ...interface{}) bool {
	if h, ok := t.(tHelper); ok {
		h.Helper()
	}
	...
	if !ok {
		return Fail(t, fmt.Sprintf("\"%s\" could not be applied builtin ordering", object), msgAndArgs...)
	}

	if ordered {
		Fail(t, fmt.Sprintf("Should be ordered, but was %v", object), msgAndArgs...)
	}
...
func IsNotOrdered(t TestingT, object interface{}, msgAndArgs ...interface{}) bool {
	if h, ok := t.(tHelper); ok {
		h.Helper()
	}
	...
	if !ordered {
		Fail(t, fmt.Sprintf("Should be not ordered, but was %v", object), msgAndArgs...)
	}
