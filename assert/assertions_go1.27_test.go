...
func TestNotErrorAsType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("NotErrorAsType[*customError](%#v)", tt.err), func(t *testing.T) {
			mockT := new(captureTestingT)
			result := NotErrorAsType[*customError](mockT, tt.err)
...
func TestErrorAsTypeGeneratedAPIs(t *testing.T) {
	...
	if target, ok := ErrorAsTypef[*customError](t, err, "message"); !ok || target == nil {
		t.Error("ErrorAsTypef did not return the matched error")
	}
	if target, ok := assertions.ErrorAsType[*customError](err); !ok || target == nil {
		t.Error("Assertions.ErrorAsType did not return the matched error")
	}
	if target, ok := assertions.ErrorAsTypef[*customError](err, "message"); !ok || target == nil {
		t.Error("Assertions.ErrorAsTypef did not return the matched error")
	}
	if !NotErrorAsTypef[*customError](t, io.EOF, "message") {
		t.Error("NotErrorAsTypef unexpectedly failed")
	}
	if !assertions.NotErrorAsType[*customError](io.EOF) {
		t.Error("Assertions.NotErrorAsType unexpectedly failed")
	}
	if !assertions.NotErrorAsTypef[*customError](io.EOF, "message") {
		t.Error("Assertions.NotErrorAsTypef unexpectedly failed")
	}

	mockT := new(captureTestingT)
	if _, ok := New(mockT).ErrorAsTypef[*customError](io.EOF, "generated message"); ok {
		t.Error("Assertions.ErrorAsTypef unexpectedly succeeded")
	}
	if !strings.Contains(mockT.msg, "generated message") {
		t.Errorf("Assertions.ErrorAsTypef did not forward its message: %q", mockT.msg)
	}
}
