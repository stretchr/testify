...
func TestRegexpWithInvalidRegex(t *testing.T) {
	// Issue #1794: Regexp should fail gracefully on invalid regex, not panic
	t.Parallel()

	mockT := new(testing.T)

	// Should not panic; should return false (assertion fails)
	False(t, Regexp(mockT, `\C`, "any string"))
	False(t, NotRegexp(mockT, `\C`, "any string"))

	// Another invalid pattern
	False(t, Regexp(mockT, `(?P<open>[)`, "test"))
	False(t, NotRegexp(mockT, `(?P<open>[)`, "test"))
}
