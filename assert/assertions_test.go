...
func TestRegexpInvalidExpression(t *testing.T) {
	...
	// Verify that an invalid regexp expression fails the test with a useful
	// error message instead of panicking (regression test for issue #1794).
	mockT := new(testing.T)

	// \C is an invalid escape sequence - previously this caused a panic via
	// regexp.MustCompile; now it should fail gracefully.
