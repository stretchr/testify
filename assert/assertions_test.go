...
func TestInEpsilonSlice(t *testing.T) {
	...
	False(t, InEpsilonSlice(inEpsilonSliceMockT, "", nil, 1, "Expected non numeral slices to fail"))
	Equal(t, "\n\tError Trace:\t\n\tError:      \tParameters must be slice\n\tMessages:   \tExpected non numeral slices to fail\n", inEpsilonSliceMockT.errorString())

	inEpsilonSliceMockT = new(mockTestingT)
	True(t, InEpsilonSlice(inEpsilonSliceMockT,
		[]float64{2.2, math.NaN(), 2.0},
		[]float64{2.1, math.NaN(), 2.1},
		0.06), "{2.2, NaN, 2.0} is element-wise close to {2.1, NaN, 2.1} in epsilon=0.06")
	Equal(t, "", inEpsilonSliceMockT.errorString())
}
