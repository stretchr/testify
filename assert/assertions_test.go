...
func TestYAMLEq_NumericBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "above maximum int32",
			expected: "2147483648",
			actual:   "2147483648.0",
			equal:    true,
		},
		{
			name:     "below minimum int32",
			expected: "-2147483649",
			actual:   "-2147483649.0",
			equal:    true,
		},
...
func TestYAMLValuesAreEqualNumbers(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "minimum int64 rounding collision", expected: int64(-1<<63 + 1), actual: float64(-0x1p63)},
		{name: "below minimum int64", expected: int64(-1 << 63), actual: belowMinInt64},
		{name: "largest exactly representable int64", expected: int64(1<<63 - 1024), actual: float64(0x1.fffffffffffffp62), equal: true},
		...
		{name: "unsigned rounding collision", expected: uint64(1<<63 + 1), actual: float64(0x1p63)},
		...
		{name: "negative zero and uint", expected: negativeZero, actual: uint64(0), equal: true},
		{name: "positive infinities", expected: math.Inf(1), actual: math.Inf(1), equal: true},
		{name: "negative infinities", expected: math.Inf(-1), actual: math.Inf(-1), equal: true},
