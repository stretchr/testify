...
func TestRegexp(t *testing.T) {
	...
	// Invalid pattern: must fail the assertion gracefully instead of panicking.
	invalidPatterns := []string{`\C`, `[invalid`}
	for _, rx := range invalidPatterns {
		mockFailed := new(testing.T)
		False(t, Regexp(mockFailed, rx, "anything"), "Regexp with invalid pattern %q should return false", rx)
		True(t, mockFailed.Failed(), "Regexp with invalid pattern %q should mark test as failed", rx)

		mockFailed = new(testing.T)
		False(t, NotRegexp(mockFailed, rx, "anything"), "NotRegexp with invalid pattern %q should return false", rx)
		True(t, mockFailed.Failed(), "NotRegexp with invalid pattern %q should mark test as failed", rx)
	}
}
