...
func (a *Assertions) Consistently(condition func(*CollectT), times int, tick time.Duration, msgAndArgs ...interface{}) bool {
	if h, ok := a.t.(tHelper); ok {
		h.Helper()
	}
	return Consistently(a.t, condition, times, tick, msgAndArgs...)
}
...
//		require.Equal(c, 7, i)
//	}, 10, 1*time.Second, "shouldError() did not return 7 and an error")
func (a *Assertions) Consistentlyf(condition func(*CollectT), times int, tick time.Duration, msg string, args ...interface{}) bool {
	if h, ok := a.t.(tHelper); ok {
		h.Helper()
	}
	return Consistentlyf(a.t, condition, times, tick, msg, args...)
}
...
//		externalValue = true
//	}()
//	a.EventuallyTimes(func(c *assert.CollectT) {
//		// add assertions as needed; any assertion failure will fail the current tick
//		assert.True(c, externalValue, "expected 'externalValue' to be true")
//	}, 10, 1*time.Second, "external state has not changed to 'true'; still false")
func (a *Assertions) EventuallyTimes(condition func(*CollectT), times int, tick time.Duration, msgAndArgs ...interface{}) bool {
	if h, ok := a.t.(tHelper); ok {
		h.Helper()
	}
	return EventuallyTimes(a.t, condition, times, tick, msgAndArgs...)
}
...
//		externalValue = true
//	}()
//	a.EventuallyTimesf(func(c *assert.CollectT, "error message %s", "formatted") {
//		// add assertions as needed; any assertion failure will fail the current tick
//		assert.True(c, externalValue, "expected 'externalValue' to be true")
//	}, 10, 1*time.Second, "external state has not changed to 'true'; still false")
func (a *Assertions) EventuallyTimesf(condition func(*CollectT), times int, tick time.Duration, msg string, args ...interface{}) bool {
	if h, ok := a.t.(tHelper); ok {
		h.Helper()
	}
	return EventuallyTimesf(a.t, condition, times, tick, msg, args...)
}
